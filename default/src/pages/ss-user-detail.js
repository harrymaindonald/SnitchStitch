...
<app-route route="{{route}}" pattern="/:ID" data="{{routeData}}" tail="{{subroute}}">
  ...
  connectedCallback() {
    ...
    this.set('errors', []);
    var collectorRef = db.collection("Projects").doc("TestingProject").collection("error_Browser").doc("item._id"); //in future once errors amount is added into db change order by to errors amount -hm
