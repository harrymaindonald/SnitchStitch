...
class SSTopOccurances extends PolymerElement {
  ...
  ready() {
    super.ready();
    this.set('errors', []); //in future once errors amount is added into db change order by to errors amount -hm

    firebase.firestore().collection("Projects").doc("TestingProject").collection("error_generic").limit(2).orderBy("instances").onSnapshot(querySnapshot => {
